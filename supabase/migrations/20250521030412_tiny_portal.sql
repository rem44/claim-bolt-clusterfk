/*
  ...
  1. Changes
    - Adds unique constraints for clients.code, products.code, and invoices.invoice_number
    - Inserts sample clients, products, and invoices with items
  ...
  3. Constraints
    - Ensures unique constraints exist before attempting inserts
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'clients_code_key'
  ) THEN
